...
const AdminReviewCard = ({
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="md:text-3xl font-bold text-light font-title text-2xl md:text-left text-center mb-4 px-0">
          {title}
        </h3>
        <div className="flex justify-between items-center mb-4">
          <p className="text-light md:text-left text-center font-bold my-3">
            <span className="text-dark">Autor:</span> {author}
          </p>
              ...
              <div className="flex mt-2">
                {[...Array(5)].map((_, i) =>
                  i < rating ? (
                    ...
                    <FaRegStar key={i} className="text-light" />
                  )
                )}
              </div>
            </div>
      ...
      >
        <img
          src={imageUrl}
          alt={title}
          className=" rounded-lg w-full h-auto drop-shadow-md object-cover"
        />
      </div>
    </div>
  );
