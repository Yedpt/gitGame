...
const EditNew = () => {
    ...
    const fetchNews = async (id) => {
        try {
            const response = await getNewById(id);  // Asegúrate de tener este servicio
            console.log("Noticia obtenida:", response); 
            setFormData({
                id: response.id,
                user_id: response.user_id,
                title: response.title,
                news: response.news,
                image_url: response.image_url,  // Esto lo necesitarías si quieres mostrar la imagen
                image2_url: response.image2_url,  // Lo mismo para la imagen secundaria
            });
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            ...
            try {
                // Usamos `updateNew` para actualizar la noticia
                await updateNew(formData.id, formDataToSend);
                alert('Noticia actualizada exitosamente');
                navigate('/news');
            } catch (error) {
                setSubmissionError('Error al actualizar la noticia: ' + error.message);
        ...
        >
            <div className="flex-grow flex justify-center items-center mt-20">
                <div className="bg-greenMid p-8 rounded-lg shadow-lg max-w-md w-full md:max-w-lg">
                    <h2 className="font-title text-white text-4xl mb-6 text-center">EDITA UNA NOTICIA</h2>
